...
export function Navbar() {
  ...
  const navLinks = [
    ...
    {
      href: "https://app.athenstennis.gr/athens-tennis/home",
      label: t("schedule"),
      external: true,
    },
  ];
    ...
    >
      ...
      >
        ...
        <div className="hidden items-center gap-8 text-md font-medium text-slate-600 md:flex">
          {navLinks.map((item) => {
            const isSchedule = item.label === t("schedule");
            const isAnchor = item.href.startsWith("#");
            const baseClasses =
              "transition-colors hover:text-primary-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 focus-visible:ring-offset-transparent";
            const scheduleClasses = isSchedule
              ? " rounded-full border border-accent-500 px-4 py-2 text-primary-500"
              : "";
          ...
          >
            <div className="flex flex-col gap-4 text-base font-medium text-slate-700">
              {navLinks.map((item) => {
                ...
                const isAnchor = item.href.startsWith("#");
                const baseClasses =
                  "rounded-2xl px-3 py-2 transition hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2";
                const scheduleClasses = isSchedule
                  ? " border border-accent-500 text-primary-500"
                  : "";
